...
function endGame(player, playerId){
    ...
    (async () => {
        if(await global.db.data.findOne("userGold", { id: playerId })){
            await global.db.data.updateOne("userGold", { id: playerId }, obj => {
                obj.gold += length;
                return obj;
            });
        }else{
            await global.db.data.add("userGold", { id: playerId, gold: length, items: [], daily: "0"});
        }
    })();
}
